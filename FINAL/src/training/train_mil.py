...
def evaluate_constants(train_loader, val_loader):
...
def train_mil_model(
    ...
    local_weights = weights_path if (weights_path and os.path.exists(weights_path)) else None
    ...
    training_config = {
        ...
        "training_mode": training_mode, "joint_margin": joint_margin,
    ...
    print(f"Loading data (manifest={manifest}, cache={cache_path})...")
    ...
    constants = evaluate_constants(train_loader, val_loader)
    print(f"Constant Baseline - Train Mean: {constants['train_mean']:.4f} -> Val MAE: {constants['mean_mae']:.4f}")
    
    print("Initializing DINOv3 MIL Regressor...")
    model = DINOv3MILRegressor(**model_config)
    model.to(device)
    
    # Optimize trainable parameters (MIL pool + head)
    ...
    entropy_criterion = AttentionEntropyLoss() if lambda_entropy > 0 else None
    
    writer = SummaryWriter(tb_dir)
    csv_log_path = os.path.join(logs_dir, "training_log.csv")
    log_columns = ['epoch', 'train_loss', 'train_mae', 'val_loss', 'val_mae', 'val_pearson', 'val_spearman', 'lr', 'time_sec']
    log_data = []
    ...
    for epoch in range(1, epochs + 1):
        ...
        denom = len(train_loader.dataset) * (2 if training_mode == 'joint' else 1)
        train_loss_epoch = train_loss_sum / denom
        train_mae_epoch = train_mae_sum / denom
        ...
        if len(all_val_preds) > 1:
            ...
            pearson_r, _ = pearsonr(all_val_targets, all_val_preds)
        else:
        ...
        print(f"Epoch {epoch:03d}/{epochs} | "
              ...
              f"Val Loss: {val_loss_epoch:.4f}, MAE: {val_mae_epoch:.4f}, r: {pearson_r:.4f}, Rho: {spearman_rho:.4f} | "
        ...
        writer.add_scalar('Pearson/Val', pearson_r, epoch)
        writer.add_scalar('Spearman/Val', spearman_rho, epoch)
        writer.add_scalar('LR', current_lr, epoch)
        
        log_data.append([epoch, train_loss_epoch, train_mae_epoch, val_loss_epoch, val_mae_epoch, pearson_r, spearman_rho, current_lr, epoch_time])
