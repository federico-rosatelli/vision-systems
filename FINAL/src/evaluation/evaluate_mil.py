...
def evaluate_mil_model(manifest, model_path, batch_size=32, out_dir=None, num_workers=4, cache_path=None):
    ...
    print(f"Loading test set from {manifest}...")
    _, _, test_loader = get_patch_dataloaders(
    ...
    # 2. Load Model
    print(f"Loading MIL Regressor from {model_path}...")
    ...
    with torch.no_grad():
        for patch_tensors, area_tensors, targets, groups in test_loader:
